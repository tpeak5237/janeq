...
test.describe("JaneQ visual evidence", () => {
  ...
  test("captures the mobile utility state", async ({ browser }) => {
    ...
    await page.evaluate(() => window.scrollTo(0, 0));
    await page.screenshot({
      path: "docs/screenshots/janeq-mobile.png",
      fullPage: true,
    });
    await page.close();
  });

  test("captures the Thai desktop state", async ({ page }) => {
    await page.goto("/");
    await page.getByRole("button", { name: "Switch to Thai" }).click();
    await page.getByLabel("ลิงก์เว็บไซต์").fill("https://theerapat.org/janeq");
    await expect(page.getByTestId("qr-preview").locator("img")).toBeVisible();
    await page.evaluate(() => window.scrollTo(0, 0));
    await page.screenshot({
      path: "docs/screenshots/janeq-thai-desktop.png",
      fullPage: true,
    });
  });
  ...
  test("captures the dark theme state", async ({ page }) => {
    ...
    await page.evaluate(() => window.scrollTo(0, 0));
    await page.screenshot({
      path: "docs/screenshots/janeq-dark.png",
      fullPage: true,
    });
  });
  ...
  test("captures the Thai mobile state", async ({ browser }) => {
    ...
    await page.evaluate(() => window.scrollTo(0, 0));
    await page.screenshot({
      path: "docs/screenshots/janeq-thai-mobile.png",
      fullPage: true,
    });
    await page.close();
  });
